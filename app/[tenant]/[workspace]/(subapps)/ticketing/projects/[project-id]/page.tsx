...
export default async function Page(props0: {
  ...
  const items = [
    workspace.config.isShowAllTickets && {
      ...
      icon: MdAllInbox,
      href: allTicketsURL,
      iconClassName: 'bg-[#fce4ec] text-[#c2185b]',
    },
    ...
    workspace.config.isShowResolvedTicket && {
      ...
      iconClassName: 'bg-mint-50 text-mint-700',
    },
  ]
    .filter(Boolean)
    .map(
  ...
  return (
    <div className="bg-ink-25 min-h-full">
      <div className="container my-6 space-y-6 mx-auto">
        ...
        />
        <div className="bg-white rounded-xl border border-ink-100 shadow-xs p-2">
          <Swipe items={items} />
        </div>
...
const wrapperClassName = 'flex items-center gap-4 px-4 py-4 h-full';
...
async function TicketCard(props: TicketCardProps) {
  ...
  return (
    <Link
      ...
      className={cn(
        wrapperClassName,
        'group hover:bg-ink-50 transition-colors rounded-lg',
      )}>
      <div className={cn(iconWrapperClassName, iconClassName)}>
        <Icon className="h-5 w-5" />
      </div>
      <div className="grow flex flex-col">
        <h3 className="text-2xl font-bold text-ink-900 tabular-nums leading-none">
          {formatNumber(count)}
        </h3>
        <p className="text-xs font-semibold text-ink-500 mt-1">{label}</p>
      </div>
    </Link>
  );
}
